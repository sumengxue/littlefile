  ...
  (let ((file (buffer-file-name)) base-name)
    (if (not file)
      ...
      (let ((extension (file-name-extension file)))
        (cond
         ((equal extension "cpp")
          (compile (format "clang++ -g -Wall %s" file )))
         ((equal extension "c")
          (compile (format "clang -g -Wall %s -o %s" file (file-name-base))))
...
(defun compile-run () (compile-buffer)(cppRun))
(global-set-key (kbd "<f11>") #'compile-run)
